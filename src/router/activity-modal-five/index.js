...
export default class ActivityModal extends React.Component {
        ...
        <div className="coupon-list">
          {
            activityConfig.limitCouponList && activityConfig.limitCouponList.map((item, index) => {
              if (activityConfig.limitCouponList.length % 2 === 1 && (index === (activityConfig.limitCouponList.length - 1 * 1))) {
                return (
                  <div key={'coupon' + index} className='coupon-list-singleimg' onClick={ this.receiveSnapCoupon.bind(this, item, index) }>
                    <img src={(item.info && item.info.reachPurchaseLimit) ? item.gotImg : (item.info && item.info.stock) ? item.img : item.soleOutImg } className='coupon-list-singleimg'/>
                  </div>
                )
              }
              return (
                <div key={'coupon' + index} className='coupon-list-img' onClick={ this.receiveSnapCoupon.bind(this, item, index) }>
                  <img src={ (item.info && item.info.reachPurchaseLimit) ? item.gotImg : (item.info && item.info.stock) ? item.img : item.soleOutImg } className='coupon-list-img'/>
                </div>
              )
              /*return (
  ...
  receiveSnapCoupon = (coupon, index) => {
